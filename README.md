...
1) uses an independent-dimensions strategy to construct a policy - ie. to use 3 policies for left/right, up/down, forward/back
2) assumes that policies are generated from a continuous-state markov decision process are [ContinuousStateValueFunctionGreedyPolicy]s (https://github.com/ml4j/ml4j-mdp/blob/master/src/main/java/org/ml4j/mdp/ContinuousStateValueFunctionGreedyPolicy.java)
3) loads the policies from serialized files, by name, from the dronez-policies project.

The goal of this project is to demonstrate test Flights for policies generated by the dronez-policies project
